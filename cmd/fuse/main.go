...
import (
	...
	fuse "example.com/p2pfs/fuse/fs"
...
func writeMemProfile(fn string, sigs <-chan os.Signal) {
	...
	for range sigs {
		...
		f, err := os.Create(fn)
		if err != nil {
			log.Infof("Create: %v", err)
			continue
		}
		pprof.WriteHeapProfile(f)
		if err := f.Close(); err != nil {
			log.Infof("close %v", err)
		}
	}
}
...
func main() {
	...
	apiAddrStr := flag.String("apiAddr", "127.0.0.1:6006", "gRPC API bind address")
	dbAddrStr := flag.String("dbAddr", "", "p2pdb source address")
	dbKeyStr := flag.String("dbKey", "", "p2pdb key")
	...
	if *memprofile != "" {
		if !*quiet {
			fmt.Printf("send SIGUSR1 to %d to dump memory profile", os.Getpid())
		}
	...
	if err := util.SetLogLevels(map[string]logging.LogLevel{
		"fs":   util.LevelFromDebugFlag(*debug),
		"fuse": util.LevelFromDebugFlag(*debug),
	}); err != nil {
		log.Fatal(err)
	}

	orig := flag.Arg(1)
	loopbackRoot, err := fuse.NewLoopbackRoot(orig, *apiAddrStr, *dbAddrStr, *dbKeyStr)
	if err != nil {
